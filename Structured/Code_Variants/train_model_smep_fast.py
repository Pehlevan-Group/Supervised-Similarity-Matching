...
from model_smep_fast import Network


def train_net(net):
    print "Learning rule: SMEP, Structured connections"
    print "Code version: Fast, validation error only every 10 epochs, doesn't save the struct matrices, all but last fwd matrix are sparse, last fwd matrix is dense"
    path = net.path
    ...
    print "Using 10% of training batches"
    n_batches_train = 50000 / batch_size
    n_batches_valid = 10000 / batch_size
    percent_trainset = 0.1
    n_batches_train  = int(n_batches_train*percent_trainset)
    ...
    for epoch in range(n_epochs):
        ...
        for index in xrange(n_batches_train):
            ...
            if index == (n_batches_train - 1):  # debug
                ...
                measures_avg_tr[
                    2] *= 100.  # measures_avg[-1] corresponds to the error rate, which we want in percentage
                stdout.write(
                    "\r%2i-train-%5i E=%.1f C=%.5f SqNorm=%.2f LinTerm=%.2f QuadTerm=%.2f QuadLatTerm=%.2f error=%.3f%%" % (
                        ...
                        measures_avg_tr[3], measures_avg_tr[4], measures_avg_tr[5], measures_avg_tr[6],
                        measures_avg_tr[2]))
                stdout.flush()
            ...
            try:
                ...
                Delta_logW, Delta_logL = np.array(wdot_listarr[:len(alphas_fwd)]), np.array(
                    wdot_listarr[len(alphas_fwd):(
                            len(alphas_fwd) + len(alphas_lat))])  # eff
            except ValueError:
                Delta_logW, Delta_logL = np.zeros(len(alphas_fwd), dtype=np.float32), np.zeros(len(alphas_lat),
                                                                                               dtype=np.float32)
