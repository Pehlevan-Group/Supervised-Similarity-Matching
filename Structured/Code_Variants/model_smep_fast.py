...
'''
Same as SMEP Str with two additions:
a) Modular for CIFAR10
b) Treats the last FC matrix as a dense matrix
'''
...
class Network(object):
    ...
    def __init__(self, name, hyperparameters=dict()):
        ...
        # Layers
        self.struct_matrices, self.biases, self.weights, self.hyperparameters, self.training_curves = self.__load_params(
            hyperparameters)
        # struct_matrices: 1-> L
        ...
        values = [np.zeros((dataset_size, int(self.hyperparameters["nps"][l_ind] * (layer_dim ** 2))),
                           dtype=theano.config.floatX) for l_ind, layer_dim in
                  enumerate(layer_dims[1:(-1)])]  # Flat choice, values of the nodes, HL=1...L, bs*Vl
    ...
    def save_params(self):  # ADAPT FOR STR
        f = file(self.path, 'wb')
        #struct_fwd_matrices, struct_lat_matrices = [s.get_value() for s in self.struct_matrices['fwd']], [s.get_value()
        #                                                                                                  for s in
        #                                                                                                  self.struct_matrices[
        #                                                                                                      'lat']]
        biases_fwd_values = [b.get_value() for b in self.biases['fwd']]
        weights_fwd_values, weights_lat_values = [W.get_value() for W in self.weights['fwd']], [W.get_value() for W in
                                                                                                self.weights['lat']]
    ...
    def __load_params(self, hyperparameters):  # R, ADAPT FOR STR, USE STRUC_FUNCS HERE
        ...
        if os.path.isfile(self.path):  # loads weights from the .save file for all subsequent epochs
            f = file(self.path, 'rb')
            struct_fwd_matrices, struct_lat_matrices, biases_fwd_values, weights_fwd_str, weights_lat_str, hyperparameters, training_curves = cPickle.load(
                f)  # assuming all lat, 1->L of W saved as sparse matrices
            f.close()
            ...
            input_dim, input_chan = 28, 1  # DEBUG
            if hyperparameters["dataset"] == "cifar10":
                input_dim, input_chan = 32, 3
            if hyperparameters["dataset"] == "mnist_reduced":
                input_dim, input_chan = 20, 1
            ...
            Vl_list[:-1] = (np.array(dl_list[:-1]) ** 2) * np.array(
                [input_chan] + nps)  # total number of nodes in each layer, eg: [4096, 3072...]
            ...
            print 'W norm: ', np.linalg.norm(np.array(weights_fwd_str[0].todense())), np.linalg.norm(
                weights_fwd_str[1]), ' W norm perelem: ', np.linalg.norm(np.array(weights_fwd_str[0].todense())) / (
                        Vl_list[0] * Vl_list[1]), np.linalg.norm(weights_fwd_str[1]) / (Vl_list[1] * Vl_list[2])
            print 'M norm: ', np.linalg.norm(np.array(weights_lat_str[0].todense())), np.linalg.norm(
                np.array(weights_lat_str[0].todense())) / (Vl_list[1] ** 2.0)
    ...
    def __energy(self, layers, longbool=0):  # Eqn 1
        ...
        quadratic_terms_penult = - sum(
            [sparse_batched_dot(sp_b.structured_dot(self.rho(pre), W), self.rho(post)) for pre, W, post in
             zip(layers[:-2], self.weights['fwd'][:-1], layers[1:(-1)])])  # STRCHANGE
        # theano.sparse.basic.structured_dot(sp0, net.weights['fwd'][0]).eval()
        quadratic_terms = quadratic_terms_penult - sum(
            [T.batched_dot(T.dot(self.rho(pre), W), self.rho(post)) for pre, W, post in
             zip([layers[-2]], [self.weights['fwd'][-1]], [layers[(-1)]])])
        ...
        if longbool == 0:
            ...
            return (
                               squared_norm + linear_terms + quadratic_terms + quad_lat), squared_norm, linear_terms, quadratic_terms, quad_lat
    ...
    def __build_measure_verbose(self):
        ...
        measure = theano.function(inputs=[], outputs=[E, C, error, Sq_n, Lin_term, Quad_term,
                                                      Quad_lat])  # STRCHANGE, Unrelated to str, removed on_unused_input warning
    ...
    def __build_free_phase(self):
        ...
        for particles, layer, layer_end in zip(self.persistent_particles, self.layers[1:], layers_end[1:]):
            updates[particles] = T.set_subtensor(layer,
                                                 layer_end)  # replaces old subtensors of layer with layer_end, assigns to updates[particles]

        free_phase = theano.function(
            # SV Inputs: particles (node values), the particles are updated according to updates calculated from theano scan
    ...
    def __build_weakly_clamped_phase(self):
        ...
        # STRCHANGE
        e_free, squared_norm, linear_terms, quadratic_terms, quad_lat = self.__energy(self.layers,
                                                                                      1)  # These are the layers post Free relaxation
        E_mean_free = T.mean(e_free)
        e_wc, squared_norm_wc, linear_terms_wc, quadratic_terms_wc, quad_lat_wc = self.__energy(layers_weakly_clamped,
                                                                                                1)
        ...
        weights_fwd_new = [W - alpha * dot for W, alpha, dot in
                           zip(self.weights['fwd'], alphas_fwd, weights_fwd_dot)]  # Eq13
        ...
        w_new_dense, l_new_dense = [theano.sparse.basic.csm_data(w) for w in weights_fwd_new[:-1]], [
            theano.sparse.basic.csm_data(l) for l in weights_lat_new]
        w_old_dense, l_old_dense = [theano.sparse.basic.csm_data(w) for w in self.weights['fwd'][:-1]], [
            theano.sparse.basic.csm_data(l) for l in self.weights['lat']]
        w_new_dense.append(weights_fwd_new[-1].flatten())
        w_old_dense.append(self.weights['fwd'][-1].flatten())

        # print type(w_new_dense[0]), type(w_new_dense[-1])
        Delta_log = [T.sqrt(((W_new - W) ** 2).mean()) / T.sqrt((W ** 2).mean()) for W, W_new in
                     zip(w_old_dense, w_new_dense)] + [T.sqrt(((W_new - W) ** 2).mean()) / T.sqrt((W ** 2).mean()) for
                                                       W, W_new in zip(l_old_dense, l_new_dense)]
        # STRCHANGE
