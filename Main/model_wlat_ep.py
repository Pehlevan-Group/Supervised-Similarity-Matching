...
class Network(object):
    ...
    def __energy(self, layers, verbose_bool=0): #Eqn 1
        squared_norm = sum([T.batched_dot(rho(layer), rho(layer)) for layer in layers]) / 2.
        linear_terms = - sum([T.dot(rho(layer), b) for layer, b in zip(layers[1:], self.biases['fwd'])]) #R
        #linear_terms -= sum([T.dot(rho(layer), b) for layer, b in zip(layers[1:(-1)], self.biases['lat'])])
        quadratic_terms = - sum([T.batched_dot(T.dot(rho(pre), W), rho(post)) for pre, W, post in
                                 zip(layers[:-1], self.weights['fwd'], layers[1:])]) #R
        quad_lat =  sum([T.batched_dot(T.dot(rho(node), W), rho(node)) for W, node in
    ...
    def __build_free_phase(self):
        ...
        def step(*layers): #Eq8, beta=0
            ...
            layers_new = [layers[0]] + [T.clip(layer + epsilon * dot, 0., 1.) for layer, dot in
    ...
    def __build_weakly_clamped_phase(self):
        ...
        weights_lat_dot = T.grad((E_mean_weakly_clamped - E_mean_free) / beta, self.weights['lat'],
                                 consider_constant=layers_weakly_clamped)
        ...
        weights_lat_new = [W - alpha * dot for W, alpha, dot in
                           zip(self.weights['lat'], alphas_lat, weights_lat_dot)]  # Eq13
