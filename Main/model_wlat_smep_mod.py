...
from external_world import External_World_Normalized
...
class Network(object):
    ...
    def __init__(self, name, hyperparameters=dict()):
        ...
        if (self.hyperparameters["dataset"] == "mnist_norm"):
            self.external_world = External_World_Normalized()
        else:
    ...
    def __build_weakly_clamped_phase(self):
        ...
        #if ('beta' in self.hyperparameters["alpha_tdep_type"]): 
        weights_lat_new = [W + alpha * (dot - W/(2.0*np.abs(beta))) for W, alpha, dot in
                               zip(self.weights['lat'], alphas_lat, weights_lat_dot)]  #SMEP
        '''else:
            a2 = min(np.float32(alpha/(2.0*np.abs(beta))), 0.9)
            weights_lat_new = [W + alpha * dot - (a2* W) for W, alpha, dot in
                               zip(self.weights['lat'], alphas_lat, weights_lat_dot)]'''  #SMEP
