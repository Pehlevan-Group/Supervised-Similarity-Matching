...
def train_net(net):
    ...
    print "Theano config check: ", theano.config.floatX
    path = net.path
    ...
    for epoch in range(n_epochs):
        ...
        if alpha_tdep_type=='alphatd1': #EXP DECR, NEW
            #alphas_lat = [np.float32(alpha /(1.0+epoch)) for alpha in alphas_lat]
            if epoch<5:
                alphas_lat = alphas_lat
            else:
                alphas_lat = list(np.zeros(len(alphas_lat), dtype=np.float32))
        if alpha_tdep_type=='alphatd2':#EXP DECR, NEW L is clamped and never changes
            #alphas_lat = [np.float32(alpha /(1.0+(0.5*epoch))) for alpha in alphas_lat]
            alphas_lat = list(np.zeros(len(alphas_lat), dtype=np.float32))
        if alpha_tdep_type=='alphainv':
            #alphas_fwd = [np.float32(alpha/(1.0 + epoch)) for alpha in net.hyperparameters["alphas_fwd"]]
        ...
        if alpha_tdep_type=='alpha_segmented_repr':
            if epoch<70:
                ...
                alphas_lat = list(np.array([0.05, 0.01, 0.005], dtype=theano.config.floatX))
        if ('adap_trerr' in alpha_tdep_type):
            if epoch>=1:
                trerr_last = net.training_curves["training error"][-1]
                if (trerr_last<0.1):
                    #alphas_fwd = [alpha/100.0 for alpha in net.hyperparameters["alphas_fwd"]]
                    alphas_lat = list(np.asarray([alpha/100.0 for alpha in net.hyperparameters["alphas_lat"]], dtype=np.float32))
                elif (trerr_last<0.5):
                    #alphas_fwd = [alpha/50.0 for alpha in net.hyperparameters["alphas_fwd"]]
                    alphas_lat = list(np.asarray([alpha/50.0 for alpha in net.hyperparameters["alphas_lat"]], dtype=np.float32))
                elif (trerr_last<1):
                    #alphas_fwd = [alpha/10.0 for alpha in net.hyperparameters["alphas_fwd"]]
                    alphas_lat = list(np.asarray([alpha/10.0 for alpha in net.hyperparameters["alphas_lat"]], dtype=np.float32))
                elif (trerr_last<5):
                    #alphas_fwd = [alpha/5.0 for alpha in net.hyperparameters["alphas_fwd"]]
                    alphas_lat = list(np.asarray([alpha/5.0 for alpha in net.hyperparameters["alphas_lat"]], dtype=np.float32))
                else:
                    #alphas_fwd = net.hyperparameters["alphas_fwd"]
                    alphas_lat = net.hyperparameters["alphas_lat"]
            if (alpha_tdep_type=='adap_trerr_betainc'):
                beta = min(np.float32(0.09*epoch  + 0.2), np.float32(2.0))
                print 'Beta evolving', beta
